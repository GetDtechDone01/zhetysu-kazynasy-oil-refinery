/**
 * Navbar.jsx
 * Fixed top navigation bar with logo, desktop links, and a mobile hamburger menu.
 * Becomes opaque on scroll (scrolled > 20px) and closes on route change.
 */

import { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { Menu, X } from 'lucide-react';
import { LOGO_URL, NAV_LINKS, COMPANY } from '../lib/constants';
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
      scrolled
        ? 'bg-[#181418]/95 backdrop-blur-md shadow-lg shadow-black/30 border-b border-[#a93134]/20'
        : 'bg-transparent'
    }`}>
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-gray-400 text-[9px] tracking-wider">BIN: {COMPANY.bin}</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1 relative z-50">
            {NAV_LINKS.map(({ path, label }) =>
              label === 'Request Service' ? (
                // CTA button style for the last nav item
                <Link
                  key={path}
                  to={path}
                ...
                <Link
                  ...
                  className={`px-4 py-2 text-sm font-medium transition-all duration-200 rounded-full relative ${
                    isActive(path) ? 'text-[#a93134]' : 'text-gray-300 hover:text-white'
                  }`}
                >
                  {label}
